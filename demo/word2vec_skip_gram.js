...
function window(size, list) {
    return range(list.length/size).map((_, offset) => {
        return range(size).map((_, index) => list[offset + index]);
    });
}

var sentences =`
ala ma kot a kot ma ale
kot nie lubia woda
pies nie lubią kot
woda jest mokra
ala jest sucha
sucha jest wtedy gdy nie mokra
kot jest mokra
pies lubi woda
`;
...
sentences = flatMap(id, sentences);
sentences = sentences.filter((x) => x);

var skipGram = window(3, sentences);
skipGram = skipGram.map(([one, two, three]) => [[two], [one, three]]);

var wordsList = unique(sentences);
